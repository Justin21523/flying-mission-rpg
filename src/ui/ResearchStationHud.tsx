    ...
    <div className="pointer-events-auto absolute bottom-24 left-1/2 z-30 -translate-x-1/2 select-none">
      {!open ? (
        <button onClick={() => setOpen(true)} className="rounded-full border border-emerald-500/60 bg-emerald-700/40 px-4 py-2 text-sm font-bold text-emerald-50 shadow-xl backdrop-blur-md hover:bg-emerald-700/60">
          🔬 Jin's Research Station · {points} pts
        </button>
      ) : (
        <div className="w-80 rounded-2xl border border-emerald-600/50 bg-slate-950/90 p-3 text-slate-100 shadow-2xl backdrop-blur-md">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-sm font-bold text-emerald-200">🔬 Research Station</span>
            <span className="rounded bg-emerald-800/50 px-2 py-0.5 text-xs">{points} pts</span>
              ...
              return (
                <div key={p.id} className={`rounded-lg border p-2 ${done ? 'border-emerald-700/40 bg-emerald-900/20' : isActive ? 'border-amber-600/50 bg-amber-900/15' : 'border-slate-700/60 bg-slate-900/50'}`}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-slate-400">{p.cost} pts{p.durationSec ? ` · ${p.durationSec}s` : ''}</span>
                  </div>
                  ...
                  <div className="mt-1 flex items-center justify-between gap-2">
                    ...
                    {isActive ? <span className="text-[10px] font-semibold text-amber-300">⏳ {Math.ceil(remaining)}s</span>
                      : done && !p.repeatable ? <span className="text-[10px] text-emerald-300">✓ researched</span>
                        : lockedBy.length > 0 ? <span className="text-[10px] text-slate-500">needs prereq</span>
                          : <button disabled={!can} onClick={() => useJinResearchStore.getState().startProject(p.id)} className="rounded bg-emerald-600/70 px-2 py-0.5 text-[10px] font-semibold text-white hover:bg-emerald-500 disabled:opacity-40">{p.durationSec ? `Research (${p.durationSec}s)` : 'Research'}</button>}
                  </div>
                  {isActive && (
          ...
          {useToolStore.getState().unlockedTools.length > 0 && (
            <div className="mt-2 text-[10px] text-slate-500">Unlocked tools equip in the Tool Belt (top-right).</div>
          )}
        </div>
      )}
    </div>
