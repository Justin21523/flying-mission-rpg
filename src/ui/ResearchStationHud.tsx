              ...
              return (
                <div key={p.id} className={`rounded-lg border p-2 ${done ? 'border-emerald-700/40 bg-emerald-900/20' : 'border-slate-700/60 bg-slate-900/50'}`}>
                  ...
                  <div className="mt-1 flex items-center justify-between">
                    ...
                    {done ? <span className="text-[10px] text-emerald-300">✓ researched</span>
                      : lockedBy.length > 0 ? <span className="text-[10px] text-slate-500">needs prereq</span>
                        : <button disabled={!can} onClick={() => useJinResearchStore.getState().spendOnProject(p.id)} className="rounded bg-emerald-600/70 px-2 py-0.5 text-[10px] font-semibold text-white hover:bg-emerald-500 disabled:opacity-40">Research</button>}
                  </div>
                </div>
              );
            })}
