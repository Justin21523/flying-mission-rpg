...
export const MissionEditorTab = () => {
  ...
  return (
    <CollectionEditor<MissionDefinition>
      ...
      makeNew={makeNew}
      getLabel={(m) => m.nameZhTW}
      renderFields={(m, update) => (
        <>
          <TextRow label="Name (zh-TW)" value={m.nameZhTW} onChange={(v) => update({ nameZhTW: v })} />
          ...
          <SelectRow label="Location" value={m.locationId} options={[none, ...locations.map((l) => ({ value: l.id, label: l.nameZhTW }))]} onChange={(v) => update({ locationId: v })} />
          <SelectRow label="NPC" value={m.npcId ?? ''} options={[none, ...npcs.map((n) => ({ value: n.id, label: n.nameZhTW }))]} onChange={(v) => update({ npcId: v || undefined })} />
          <SelectRow label="Route" value={m.routeId ?? ''} options={[none, ...routes.map((r) => ({ value: r.id, label: r.nameZhTW }))]} onChange={(v) => update({ routeId: v || undefined })} />
          ...
          <TextRow label="Summary (zh-TW)" area value={m.summaryZhTW} onChange={(v) => update({ summaryZhTW: v })} />
