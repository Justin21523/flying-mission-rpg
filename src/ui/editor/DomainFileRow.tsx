...
import { type EditorContentDomain, exportDomainFile, exportDomainExample, importDomainFile } from '../../game/editor/editorContentRegistry';
import { downloadJson } from './downloadJson';

// Kit — one row of the per-domain content tool: Export current JSON, download an annotated Example (schema +
// valid-id reference for external tools), Import (file → that domain), Reset.
export const DomainFileRow = ({ domain, onMsg }: { domain: EditorContentDomain; onMsg: (m: string) => void }) => {
  ...
  const doExample = () => {
    const file = exportDomainExample(domain.id);
    if (!file) return;
    downloadJson(`r3f-rpg-${domain.id}.example.json`, file);
    onMsg(`📋 Example for "${domain.label}" downloaded (schema + valid-id reference for ChatGPT etc.).`);
  };
  ...
  return (
    <div className="flex items-center gap-2 rounded bg-slate-900/50 px-2 py-1.5 text-xs">
      ...
      <button onClick={doExport} title="Download current content as JSON" className="rounded-md border border-violet-600/50 bg-violet-600/20 px-2 py-1 font-semibold text-violet-100 hover:bg-violet-600/35">⬇</button>
      <button onClick={doExample} title="Download an example/template JSON (schema + valid ids for external tools)" className="rounded-md border border-sky-600/50 bg-sky-600/20 px-2 py-1 font-semibold text-sky-100 hover:bg-sky-600/35">📋</button>
