...
export const HintsPanel = () => {
  ...
  return (
    <PanelCard title={t('panel_hints')} icon="💡" onClose={() => useUiStore.getState().toggleHints()} width="24rem">
      <div className="space-y-2 text-xs">
        <div className="rounded bg-slate-900/60 p-2">
          <div className="mb-1 font-semibold text-slate-300">{t('hints_controls')}</div>
          <p className="text-slate-400">{t('hints_controlsText')}</p>
        </div>
        <div className="rounded bg-slate-900/60 p-2">
          <div className="mb-1 font-semibold text-slate-300">{t('hints_activeObjectives')}</div>
          {active.length === 0 ? <p className="text-slate-500">{t('hints_noQuests')}</p> : (
