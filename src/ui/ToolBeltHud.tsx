  ...
  if (!def) {
    return (
      ...
      >
        ...
        <span className="text-[9px]">{t('empty')}</span>
      </div>
    );
  }
...
function ToolPicker({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-400">{t('selectTool')}</div>
      {unlockedTools.length === 0 && (
        <div className="text-xs text-slate-500">{t('noToolsTitle')}<br />{t('noToolsHint')}</div>
      )}
