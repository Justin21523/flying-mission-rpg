...
import { STORY_DIALOGUES } from '../../data/game/storyDialogues';
...
export function getDialogueTree(id: string | null | undefined): DialogueTree | undefined {
  ...
  return getEditorDialogueTree(id) ?? SEED_DIALOGUES.find((t) => t.id === id) ?? GAME_DIALOGUES.find((t) => t.id === id) ?? STORY_DIALOGUES.find((t) => t.id === id);
}
...
export function listDialogueTreeIds(): { id: string; source: 'editor' | 'seed' }[] {
  ...
  for (const t of [...SEED_DIALOGUES, ...GAME_DIALOGUES, ...STORY_DIALOGUES]) {
    if (!seen.has(t.id)) {
      seen.add(t.id);
