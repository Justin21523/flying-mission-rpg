...
// How far INSIDE the arrival edge to land — small, so you appear RIGHT AT the boundary (not pushed toward the
// centre). Teleports/portals require [E] + a travel guard, so there's no immediate-re-cross risk to avoid.
const BOUNDARY_INSET = 3;

type Vec3 = { x: number; y: number; z: number };

// Open spot right inside one boundary edge of an area. north=-z, south=+z, east=+x, west=-x.
export function spawnNearBoundary(areaId: string, edge: EdgeDir): Vec3 {
  const d = Math.max(2, getEffectiveAreaSize(areaId) - BOUNDARY_INSET);
  switch (edge) {
