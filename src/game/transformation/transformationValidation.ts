...
export function validateTimeline(def: TransformationDefinition, knownCharacterIds?: ReadonlySet<string>): string[] {
  ...
  if (def.stages.length === 0) e.push('Timeline needs at least one stage.');
  ...
  if (!def.stages.some((s) => FORM_STAGE_TYPES.has(s.type))) e.push('Needs at least one form/part/animation stage (model-visibility / model-swap / animation-clip / part-transform).');
  const exit = def.stages.find((s) => s.type === 'exit-stage');
  if (!def.stages.some((s) => s.type === 'finish-pose') && !exit) e.push('Needs a finish-pose or exit-stage.');
  ...
  if (cs.planeControllerDisableTime > cs.robotControllerEnableTime + EPS) e.push('Plane controller must disable before the robot controller enables.');
  const ps = def.physicsSwitchConfig;
  if (!ps.transitionalCollider && ps.planeColliderDisableTime > ps.robotColliderEnableTime + EPS) e.push('Plane collider must disable before the robot collider enables (or set a transitional collider).');
