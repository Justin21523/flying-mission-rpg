...
import type { VehicleDefinition, TrafficSignalDef, RoadPath } from '../types/traffic';

// Editable traffic (🚦 Traffic tab): vehicles, signals, and roads (waypoint loops). Seeded from the
// built-in data; CRUD + per-id field edits + road waypoint edits; auto-persisted. Runtime (trafficStore,
// TrafficLayer) reads these via the accessors. Roads are stored as id/areaId/waypoints and the RoadPath
// (with lengths) is recomputed on demand.
export interface EditorRoad { id: string; areaId: string; waypoints: [number, number, number][] }
...
interface EditorTrafficState {
  ...
  importState: (data: { vehicles?: VehicleDefinition[]; signals?: TrafficSignalDef[]; roads?: EditorRoad[] }) => void;
  reset: () => void;
}
...
function persist(s: Pick<EditorTrafficState, 'vehicles' | 'signals' | 'roads'>): void {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ vehicles: s.vehicles, signals: s.signals, roads: s.roads })); } catch { /* ignore */ }
}
function seedRoads(): EditorRoad[] {
  return POLI_ROADS.map((r) => ({ id: r.id, areaId: r.areaId, waypoints: clone(r.waypoints) }));
}
function load(): Pick<EditorTrafficState, 'vehicles' | 'signals' | 'roads'> {
  try {
  ...
  return { vehicles: clone(POLI_VEHICLES), signals: clone(TRAFFIC_SIGNALS), roads: seedRoads() };
}
...
export const useEditorTrafficStore = create<EditorTrafficState>((set, get) => {
  ...
  return {
    ...
    reset: () => { set({ vehicles: clone(POLI_VEHICLES), signals: clone(TRAFFIC_SIGNALS), roads: seedRoads() }); save(); },
  };
});
