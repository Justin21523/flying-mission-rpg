...
export interface FlightCollect { x: number; y: number; z: number; exp: number; coin: number; combo: number }
...
interface FlightScoreState {
  ...
  boostUntil: number;
  collected: number;
  award: (def: FlightEventDef, pos: [number, number, number]) => void;
  boostActive: () => boolean;
  reset: () => void;
}
...
export const useFlightScoreStore = create<FlightScoreState>((set, get) => ({
  ...
  award: (def, pos) => {
    ...
    const { exp, coin } = rewardFor(def, combo);
    useProgressionStore.getState().addExp(exp);
    useWalletStore.getState().addCoins(coin);
    queue.push({ x: pos[0], y: pos[1], z: pos[2], exp, coin, combo });
