...
// at boot. Read by the boss runtime via the accessors below.
export const useBossDefinitionStore = createEditorCollection<BossDefinition>({ storageKey: 'aero-rescue-editor-boss-def-v1', seed: SEED_BOSSES, makeId: () => `boss_${nanoid(6)}` });
export const useBossPhaseStore = createEditorCollection<BossPhaseDefinition>({ storageKey: 'aero-rescue-editor-boss-phase-v1', seed: SEED_BOSS_PHASES, makeId: () => `bphase_${nanoid(6)}` });
export const useBossWeakpointStore = createEditorCollection<BossWeakpointDefinition>({ storageKey: 'aero-rescue-editor-boss-weakpoint-v1', seed: SEED_BOSS_WEAKPOINTS, makeId: () => `wp_${nanoid(6)}` });
export const useBossAttackStore = createEditorCollection<BossAttackPatternDefinition>({ storageKey: 'aero-rescue-editor-boss-attack-v1', seed: SEED_BOSS_ATTACK_PATTERNS, makeId: () => `atk_${nanoid(6)}` });
export const useBossArenaStore = createEditorCollection<BossArenaDefinition>({ storageKey: 'aero-rescue-editor-boss-arena-v1', seed: SEED_BOSS_ARENAS, makeId: () => `arena_${nanoid(6)}` });
export const useBossSummonWaveStore = createEditorCollection<BossSummonWaveDefinition>({ storageKey: 'aero-rescue-editor-boss-wave-v1', seed: SEED_BOSS_SUMMON_WAVES, makeId: () => `wave_${nanoid(6)}` });
